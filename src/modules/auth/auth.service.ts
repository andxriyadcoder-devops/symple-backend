...
export class AuthService {
  ...
  async register(data: RegisterDto) {
    ...
    const updated = await this.repository.update  (user._id.toString(), {
    walletId: wallet._id,
    });
    if (data.referralCode) {
  const referrer = await this.repository.findByReferralCode(
    data.referralCode,
  );
  ...
  if (referrer) {
    ...
    const updatedUser = await this.repository.findByEmailOrUsername(
      user.username,
    );
